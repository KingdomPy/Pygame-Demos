...
class Scene:
    ...
    def __init__(self, resolution, fps, debug, data=None):
        ...
        self.chunk_size = 1000  # width, length = 1200
        self.tile_map = []
        self.spawn = (0, 0)
        self.goal = (0, 0)
        ...
        self.background = pygame.image.load("assets/maps/test.png").convert()
    ...
    def import_tile_map(self):
        ...
        if path != "":
            ...
            for chunk in self.chunks:
                chunk[0] = (chunk[0][0] * self.chunk_size, chunk[0][1] * self.chunk_size)
    ...
    def update(self, surface, events, dt, current_fps):
        ...
        split_camera_1_entity_data = []
        split_camera_1_chunk_data = []
        split_camera_2_entity_data = []
        split_camera_2_chunk_data = []
        ...
        for entity in self.entities:
            ...
            for chunk in self.chunks:
                chunk_x, chunk_y = chunk[0]
                chunk_rect = (chunk_x, chunk_y, self.chunk_size, self.chunk_size)
                if collide_rect(my_rect, chunk_rect):
                    for i in range(1, len(chunk)):
                        tile = chunk[i]
                        if tile.collidable:
                            distance = math.sqrt((entity.x - tile.x) ** 2 + (entity.y - tile.y) ** 2) - entity.width - tile.width
                            if tile.angle == 0 and entity.angle == 0 and distance <= 0:
                                tile_rect = tile.get_rect()
                                if collide_rect(my_rect, tile_rect):
                                    if tile.sprite_code == 4:
                                        entity.x = self.spawn[0]
                                        entity.y = self.spawn[1] + entity.length * 2
                                    else:
            ...
            if self.display_mode == 0:
                if self.main_camera.in_camera(position, radius):
                    camera_entity_data.append(entity.get_polygon())

            elif self.display_mode == 1:
                if self.split_camera_1.in_camera(position, radius):
                    split_camera_1_entity_data.append(entity.get_polygon())

                if self.split_camera_2.in_camera(position, radius):
                    split_camera_2_entity_data.append(entity.get_polygon())

        if self.display_mode == 0:

            for chunk in self.chunks:
                x, y = chunk[0][0] + self.chunk_size / 2, chunk[0][1] + self.chunk_size / 2
                if self.main_camera.in_camera((x, y), self.chunk_size / 2):
                    camera_chunk_data.append(chunk)

            self.main_camera.update(dt)
            self.main_camera.update_screen(surface, camera_chunk_data, camera_entity_data)
            ...
            for chunk in self.chunks:
                x, y = chunk[0][0] + self.chunk_size / 2, chunk[0][1] + self.chunk_size / 2
                if self.split_camera_1.in_camera((x, y), self.chunk_size / 2):
                    split_camera_1_chunk_data.append(chunk)
                if self.split_camera_2.in_camera((x, y), self.chunk_size / 2):
                    split_camera_2_chunk_data.append(chunk)

            self.split_camera_1.update(dt)
            self.split_camera_1.update_screen(self.half_surface_1, split_camera_1_chunk_data, split_camera_1_entity_data)
            surface.blit(self.half_surface_1, (self.resolution[0] / 4, 0))
            self.split_camera_2.update(dt)
            self.split_camera_2.update_screen(self.half_surface_2, split_camera_2_chunk_data, split_camera_2_entity_data)
...
class Camera:
    ...
    def update_screen(self, surface, chunk_data, entity_data):
        chunk_count = 0
        tile_count = 0
        ...
        text = self.main_font.render("CHUNKS:" + str(chunk_count), True, (0, 0, 0))
        surface.blit(text, (10, 40))

        text = self.main_font.render("TILES:" + str(tile_count), True, (0, 0, 0))
