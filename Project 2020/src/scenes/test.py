...
class Scene:
    ...
    def __init__(self, resolution, fps, debug, player_data=None):
        ...
        self.fontMain = pygame.font.Font(filePath.setPath(asset_path, ["fonts", "Coda-Regular.ttf"]), 24)
        self.fontSub = pygame.font.Font(filePath.setPath(asset_path, ["fonts", "Coda-Regular.ttf"]), 20)
        ...
        self.emblems = [
            pygame.image.load(filePath.setPath(asset_path, ["player", "emblems", "vanguard.png"])).convert_alpha(),
            pygame.image.load(filePath.setPath(asset_path, ["player", "emblems", "engineer.png"])).convert_alpha(),
            pygame.image.load(filePath.setPath(asset_path, ["player", "emblems", "stealth.png"])).convert_alpha(),
        ]

        for i in range(len(self.emblems)):  # Scale images
            self.emblems[i] = pygame.transform.scale(self.emblems[i], self.resolution)
        ...
        self.startupImage = pygame.transform.scale(self.startupImage, self.resolution)
        ...
        self.cursor = pygame.image.load(filePath.setPath(asset_path, ["background", "cursor.png"]))
        ...
        self.uiBoxCalculations = [
            ...
            ((0, self.resolution[1] / 3 - 5), (self.resolution[0], self.resolution[1] / 3 - 5)),
            ((0, 2 * self.resolution[1] / 3), (self.resolution[0], 2 * self.resolution[1] / 3)),
            (self.resolution[0] / 2, 2 * self.resolution[1] / 3 - 25),
            (self.resolution[0] / 2, self.resolution[1] / 3 + 30),
            self.resolution[1] / 3 - 110,
            self.resolution[1] / 3 + 70,
            self.resolution[0] * 0.25,
        ]
            ...
            if self.instance == "startGame":
                ...
                if self.clock < 30 * 16 * self.fpsScale:  # Emulate for loop
                    ...
                    if not self.drawnLoadingText:
                        ...
                        return "switch", "test", None  # Exit the scene
    ...
    def draw_ui_box(self, surface, mouse_coordinates, title="Title", text="text", options=[("Test1", "Hover on me.")], buttonScripts=[]):
        ...
        boxShift = (boxSpace - numOfOptions * 35) / 2 + self.uiBoxCalculations[
            6]  # Creates equal gap between title and the text
            ...
            if optionBox.collidepoint(mouse_coordinates):  # change colours if collinding with mouse
                ...
                if buttonScripts[i] == "":  # Check if it is an ordinary button
                    ...
                    pygame.draw.rect(surface, (168, 24, 11), (
                        optionBox[0] - 3, optionBox[1] - 3, optionBox[2] + 6, optionBox[3] + 6))  # Highlighted border
                    ...
                    if script[0] == "TOGGLE":
                        if script[1] == 1:
                            ...
                            pygame.draw.rect(surface, (234, 79, 18),
                                             (optionBox[0] - 3, optionBox[1] - 3, optionBox[2] + 6, optionBox[3] + 6))
                        else:
            ...
            if self.instance != "confirm":
                ...
                if self.cursorTouching[1] == "Yes":
                    ...
                    if self.fileType == "Normal":
                        ...
                        difficulty = "2"
                    self.playerData = [int(difficulty), 0, 0, -1, [-1, -1], [], 0,
                                       {"commands": ["Surge"]}]  # Store the player's save data to be used on launch
