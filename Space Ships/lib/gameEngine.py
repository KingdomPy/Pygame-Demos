...
class engine:
    def __init__(self, surface, dimensions, controls, debug=False, SERVER_IP="192.168.0.2", SERVER_PORT=8080):
        ...
        self.entityGroups = {"enemy":("square","triangle","entity"), "ally":("player",)}
    ...
    def addEntity(self, tag, stats={}, position=(0,0,0), size=30):
        if tag == "player":
            ...
            entity = objects.entity(tag, stats, position, size, self.dimensions)
            ...
            for attack in data:
                if attack[0] == "shotgun":
                    ...
                    for enemy in self.nearbyEnemies:
                        x,y,size = enemy
                        self.animations.append(objects.animation(attack[0], attack[1], [x,y,0], size))
                elif attack[0] == "arcanum":
                    angle = 0
                    attack[1]["range"] = self.player.size + 30
                    for i in range(8):
                        angle += math.pi/4
                        bullets.append(objects.bullet(attack[0], attack[1], (0,0,angle), attack[3]))
                else:
                    bullets.append(objects.bullet(attack[0], attack[1], attack[2], attack[3]))
    ...
    def handleInput(self, button):
        ...
        if self.instance == "map":
            ...
            if button in self.controls["L1"]:
                if self.player.commandOptions[self.selectedMove] != "": #Prevents looping
                ...
                if self.baracksOption[0] != "":
                    ...
                    if self.baracksOption[0] == "Abilities":
                        if len(self.baracksOption) == 5:
                            ...
                            if self.debug:
                                print(self.party[0].commands)
                                #print(self.party[1].commands)
    ...
    def update(self):
        ...
        if not self.returnToMenu:
            ...
            if self.networkType == "Host":
                while i < len(self.entities): 
                    ...
                    if update != 0: #0 = signal to delete the object
                        ...
                        if self.entities[i].tag in self.entityGroups["enemy"]: #if near the player add to nearby enemies
                            x,y,size = self.entities[i].x,self.entities[i].y,self.entities[i].size
                            if abs(self.player.x-x) <= self.dimensions[0]/2+size or abs(self.player.y-y) <= self.dimensions[1]/2+size:
                                self.nearbyEnemies.append((x,y,size))
                        i += 1
                    else:
                        #Remove dead enemies
                        self.entities.pop(i)
                ...
                while i < len(self.animations):
                    if self.animations[i].tag in self.playerAnimations:
                        ...
                        update = self.animations[i].update({"isRunning":"","position":(0,0)},self.instance)
                    if update != 0:
                        cameraData.append((self.animations[i].tag,self.animations[i].renderData(),self.animations[i].size))
                        i +=1
                    else:
                        if self.animations[i].tag == "orbit": #spawn a bullet where it is
                            x,y,attack = self.animations[i].x,self.animations[i].y,self.animations[i].attack
                            self.projectiles.append(objects.bullet("orbit",self.animations[i].stats,(x,y,0),70))
            ...
            else: #If the connected to another user
                #Send data to host
                x,y,angle = self.player.x,self.player.y,self.player.angle
                self.networkSocket.send(json.dumps([x,y,angle]).encode())
                
                entity = [0]
                while entity[0] != "1": #Signal to stop waiting for data
                    entity = self.networkSocket.recv(8192)
                    entity = json.loads(entity.decode())
                    cameraData.append(entity)
                cameraData.pop()
                
                #Get the nearest location and npc
                for i in range(len(cameraData)):
                    if cameraData[i][0] == "location" or cameraData[i][0] == "npc":
                        x,y = cameraData[i][1][0]
                        size = cameraData[i][2]
                        colour = cameraData[i][1][2]
                        distance = math.sqrt((self.player.x -x)**2 + (self.player.y -y)**2)
                        if distance <= size*5:
                            if cameraData[i][0] == "location":
                                if nearest_location[0] == -1:
                                    nearest_location = (distance, "Multiplayer Test", colour)
                                elif nearest_location[0] > distance:
                                    nearest_location = (distance, "Multiplayer Test", colour)
                            else:
                                if nearest_interaction[0] == -1:
                                    nearest_interaction = (distance, "Multiplayer Test", colour, (x,y), size,["Multiplayer name"],colour, "Converse")
                                elif nearest_interaction[0] > distance:
                                    nearest_interaction = (distance, "Multiplayer Test", colour, (x,y), size,["Multiplayer name"],colour, "Converse")
            ...
            #Pass data to camera
            miniMapData = self.camera.render(cameraData,self.networkType)
        ...
        while True:
            try:
                ...
                if not(address in self.connectedAddresses):
                    ...
                    entity = objects.player(str(address), self.player.stats, (0,0,0), 35)
                    self.multiplayerEntities.append(entity)
...
class camera:
    ...
    def render(self, data, networkType="Host"):
        if self.debug == True:
